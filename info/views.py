...
def timetable(request, class_id):
    ...
    matrix = [['' for i in range(7)] for j in range(6)]
...
def t_timetable(request, teacher_id):
    ...
    class_matrix = [[True for i in range(7)] for j in range(6)]
    for i, d in enumerate(DAYS_OF_WEEK):
        t = 0
        for j in range(7):
            if j == 0:
...
def marks_list(request, stud_id):
    ...
    for ass in ass_list:
        try:
            ...
            sc.save()
            sc.marks_set.create(type='I', name='Поточна 1')
            sc.marks_set.create(type='I', name='Поточна 2')
            sc.marks_set.create(type='I', name='Поточна 1')
            sc.marks_set.create(type='E', name='Атестація 1')
            sc.marks_set.create(type='E', name='Атестація 2')
            sc.marks_set.create(type='S', name='Семестрова')
        sc_list.append(sc)
